...
class OperatorNonRaceSingle extends StatefulWidget {
  const OperatorNonRaceSingle({Key key, this.category, this.documentId, this.table}) : super(key: key);
  final String category;
  final String documentId;
  final String table;

  @override
  State<OperatorNonRaceSingle> createState() => _OperatorNonRaceSingleState();
}

class _OperatorNonRaceSingleState extends State<OperatorNonRaceSingle> {
  int myNumber = 0;
  ...
  bodyWidget() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        if (document['table'] == widget.table) {
          myNumber = document['current'];
        }
        return Container(
          ...
          child: Column(
            ...
            children: [
              FittedBox(
                child: Text(
                  '${document.id.toUpperCase()} ${widget.table.toUpperCase()}',
                      ...
                      ? ElevatedButton(
                          ...
                          onPressed: () {
                            ...
                            FirebaseFirestore.instance
                                ...
                                .doc(document.id)
                                .update({'current': current, 'table': widget.table});
                          },
                        )
                      : const SizedBox(),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      FirebaseFirestore.instance
                          .collection('repeat')
                          .doc(widget.category)
                          .update({'category': widget.documentId, 'number': myNumber, 'table': widget.table});
                    },
                  ),
                ],
              ),
